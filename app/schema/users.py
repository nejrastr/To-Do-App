...
class UserSchema(SQLAlchemyAutoSchema):
    ...
    username = fields.String(
        validate=lambda x: len(x) > 4 and len(x) < 20, required=True
    )
    password = fields.String(validate=lambda x: len(x) > 8, required=True)
    ...
    def validate_password_strength(self, value):
        ...
        if not re.search(r"[!@#$%^&*()_+{}\[\]:;\"\'<>,.?/~`\\|-]", value):
            raise ValidationError(
                "Password must contain at least one specia character."
            )
